...
高速・軽量なPDFビューアー＆エディタ。Tauri v2 + React + PDF.js 製。

<p align="center">
  <img src="docs/screenshots/demo-annotate.gif" alt="注釈デモ" width="80%">
</p>
...
<table>
  <tr>
    <td align="center" width="50%">
      <img src="docs/screenshots/02-viewer.png" alt="PDFビューアー"><br>
      <sub><b>PDFビューアー</b> — 連続スクロール・サムネイルサイドバー・ズームコントロール</sub>
    </td>
    <td align="center" width="50%">
      <img src="docs/screenshots/08-annotations-on-page.png" alt="注釈機能"><br>
      <sub><b>注釈</b> — テキストボックス・ハイライト・図形・付箋をページ上に配置</sub>
    </td>
  </tr>
  <tr>
    <td align="center">
      <img src="docs/screenshots/07-textbox.png" alt="テキストボックス"><br>
      <sub><b>テキストボックス</b> — フォント・サイズ・色・太字/斜体のインラインツールバー</sub>
    </td>
    <td align="center">
      <img src="docs/screenshots/12-translate-dialog.png" alt="AI翻訳"><br>
      <sub><b>AI翻訳</b> — DeepL・OpenAI・Claude APIでページを翻訳</sub>
    </td>
  </tr>
</table>
...
|---|---|---|---|---|---|
| **価格** | 無料 / オープンソース | 高額サブスク | 無料〜サブスク | 無料（自己ホスト） | 様々 |
...
- ページサムネイルサイドバー（現在ページをアクセント色でハイライト、自動スクロール）— **サムネイルをドラッグしてページ順を変更可能**
- しおり・目次パネル（展開/折りたたみツリー、現在ページインジケーター付き）
- 全文検索（前後ナビゲーション付き）
- **インライン検索バー** — Ctrl+F / Cmd+F でフローティング検索バーを表示、Enter/Shift+Enter で移動、Esc で閉じる
...
- **コメント** — 注釈を右クリック → 「コメントを編集」でメモを付与、注釈一覧にプレビュー表示
- **右クリックコンテキストメニュー** — テキスト選択後の右クリックでコピー、注釈の右クリックで削除/コメント編集
- **スマート配置** — テキストボックス配置時に背景色・フォントサイズ・フォントファミリを周囲のPDFコンテンツから自動検出
- **Undo / Redo** — Ctrl+Z / Ctrl+Y（無制限）
- **注釈一覧パネル** — サイドバー注釈タブで全注釈をコメントプレビュー付きでリスト表示・クリックでジャンプ
- **プロパティパネル** — 選択中注釈の色・サイズ・透明度を編集
- **注釈エクスポート/インポート** — `.annot` JSON形式で保存・復元

### 編集
- 既存テキスト編集（オーバーレイ方式）
- ページ並び替え・削除（ページ整理ダイアログまたはサムネイルを直接ドラッグ）
- ページ回転（ページ単位で90°回転）
- PDF結合（複数ファイルを1つに）
- PDF分割（ページ範囲指定）
- 白紙ページ挿入
